...
class SafeAutonomousExplorer:
    def __init__(self):
        ...
        self.log_file = os.path.join(self.map_save_path, f"explorer_log_{int(time.time())}.txt")
        self.setup_logging()
    ...
    def setup_logging(self):
        ...
        try:
            ...
            logger = logging.getLogger('safe_explorer')
            ...
            rospy.loginfo(f"Logging configured. Log file: {self.log_file}")
        except Exception as e:
    ...
    def start_yolo_detector(self, retry_count=3):
        ...
        try:
            ...
            if self.yolo_process and self.yolo_process.poll() is None:
                rospy.loginfo("YOLO detector already running")
                return
                import psutil
            available_memory = psutil.virtual_memory().available / (1024 * 1024)  # MB
            if available_memory < 2000:  # 2GB minimum
                rospy.logwarn(f"Low memory available ({available_memory:.0f}MB). YOLO may not perform optimally")
            ...
            import threading
            def monitor_yolo_resources():
                while self.yolo_process and self.running:
                    try:
                        process = psutil.Process(self.yolo_process.pid)
                        cpu_percent = process.cpu_percent()
                        memory_info = process.memory_info()
                        
                        if cpu_percent > 90 or memory_info.rss > 4*1024*1024*1024:  # 4GB
                            rospy.logwarn("YOLO detector using excessive resources")
                    except Exception as e:
                        rospy.logwarn(f"Failed to monitor YOLO resources: {e}")
                    time.sleep(30)  # Check every 30 seconds
                    
            resource_monitor = threading.Thread(target=monitor_yolo_resources)
    ...
    def detect_stairs(self, depth_image, header):
        """Enhanced stair detection with performance optimization and better visualization"""
        try:
            ...
            if len(points_3d) < 1000:
                rospy.logwarn(f"Insufficient points for reliable stair detection: {len(points_3d)}")
                return
                
            # Add statistical outlier removal
            mean = np.mean(points_3d, axis=0)
            std = np.std(points_3d, axis=0)
            inlier_mask = np.all(np.abs(points_3d - mean) < 3 * std, axis=1)
            ...
            rospy.logdebug(f"Stair detection completed in {processing_time:.3f} seconds")
    ...
    def start_recovery_behavior(self):
        """Implement recovery behavior for safety-critical situations"""
        try:
            rospy.logwarn("Initiating recovery behavior")
            ...
            if not sensor_status['all_healthy']:
                rospy.logerr("Sensor health check failed during recovery")
                self.publish_status("SENSOR_ERROR")
                return False
            
            # Perform slow rotation to scan surroundings
            self.rotate_and_scan()
            
            # Reset movement lock if everything is okay
            self.movement_lock = False
            rospy.loginfo("Recovery behavior completed successfully")
            return True
            
        except Exception as e:
            rospy.logerr(f"Error in recovery behavior: {e}")
            self.emergency_stop_flag = True
            return False
            
    def check_sensor_health(self):
        """Check health of all critical sensors"""
        status = {
            'lidar': False,
            'depth_camera': False,
            'all_healthy': False
        ...
        # Check LiDAR
        if self.laser_scan and (rospy.Time.now() - self.last_lidar_time) < rospy.Duration(5.0):
            status['lidar'] = True
            
        # Check depth camera
        if hasattr(self, 'last_depth_time') and \
           (rospy.Time.now() - self.last_depth_time) < rospy.Duration(5.0):
            status['depth_camera'] = True
            
        status['all_healthy'] = all([status['lidar'], status['depth_camera']])
        return status
