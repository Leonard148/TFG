...
class SafeAutonomousExplorer:
    def __init__(self):
        ...
        try:
            rospy.loginfo("Esperando transformada inicial de 'map' a 'base_link'...")
            self.tf_listener.waitForTransform("map", self.base_frame, rospy.Time(0), rospy.Duration(10.0))
        ...
        if self.min_safe_distance <= self.critical_distance:
            rospy.logwarn("min_safe_distance debe ser mayor que critical_distance")
        ...
        self.map_save_interval = rospy.Duration(300)  # 5 minutos entre guardados
        
        # Crear directorio con timestamp
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        self.current_map_directory = os.path.join(self.map_save_path, f"map_session_{timestamp}")
        try:
        ...
        self.status_pub = rospy.Publisher('/explorer/status', String, queue_size=5)  # Status publisher
        ...
        self.depth_sub = rospy.Subscriber('/k4a/depth/image_raw', Image, self.depth_callback, queue_size=1, buff_size=2**24)
    ...
    def system_diagnostic_check(self, event=None):
        ...
        try:
            ...
            time_since_last_scan = (rospy.Time.now() - self.last_lidar_time).to_sec()
            if time_since_last_scan > 5.0:
                rospy.logwarn(f"No LiDAR data received in {time_since_last_scan:.1f} seconds")
    ...
    def depth_to_pointcloud(self, depth_image):
        ...
        try:
            ...
            if len(valid_points) < 100:
                rospy.logwarn(f"Pocos puntos válidos detectados: {len(valid_points)}")
    ...
    def should_find_frontier(self):
        ...
        if hasattr(self, "latest_grid_map"):
            self.map_coverage = self.estimate_map_coverage(self.latest_grid_map)
    ...
    def return_to_initial_position(self):
        ...
        try:
            ...
            try:
                current_trans, current_rot = self.tf_listener.lookupTransform("map", self.base_frame, rospy.Time(0))
            ...
            speed_factor = min(1.0, distance / 2.0)  # Reducir velocidad cuando está cerca
            cmd_vel.linear.x = 0.5 * speed_factor  # Velocidad máxima de 0.5 m/s
            
            # Publicar comando
            self.cmd_vel_pub.publish(cmd_vel)
            
            rospy.loginfo(f"Retornando a posición inicial. Distancia: {distance:.2f}m")
            return True
    ...
    def is_at_initial_position(self):
        ...
        try:
            ...
            current_trans, _ = self.tf_listener.lookupTransform("map", self.base_frame, rospy.Time(0))
    ...
    def save_current_map(self):
        """Guarda el mapa actual con nombre único"""
        try:
            ...
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            map_filename = f"map_{timestamp}_{self.map_save_counter}"
            
            # Guardar mapa de ocupación
            if hasattr(self, 'current_grid_map'):
                grid_map_path = os.path.join(self.current_map_directory, f"{map_filename}_grid.pgm")
                cv2.imwrite(grid_map_path, self.current_grid_map)
                
            # Guardar datos de RTAB-Map
            rtabmap_path = os.path.join(self.current_map_directory, f"{map_filename}_rtabmap.db")
            ...
            metadata_path = os.path.join(self.current_map_directory, f"{map_filename}_metadata.json")
            with open(metadata_path, 'w') as f:
                json.dump(metadata, f, indent=4)
                
            self.map_save_counter += 1
            rospy.loginfo(f"Mapa guardado exitosamente en: {self.current_map_directory}")
            return True
            
        except Exception as e:
            rospy.logerr(f"Error guardando el mapa: {e}")
            return False
