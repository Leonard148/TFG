...
class LidarExplorerWithYOLO:
    ...
    def start_yolo_detector(self):
        """Iniciar el detector YOLO como un proceso separado"""
        try:
            # Construir comando para YOLO
            yolo_script_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "yolov7/detect-mapa1.py")
            
            # Si no existe la ruta específica, intentar encontrar el script en otras ubicaciones comunes
            if not os.path.exists(yolo_script_path):
                possible_paths = [
                    "/home/jetson/catkin_ws/src/yolov7/detect-mapa1.py",  # Ruta común en entorno Jetson
                    os.path.expanduser("~/yolov7/detect-mapa1.py"),       # Home del usuario
                    "./detect_timed.py"                                   # Directorio actual
                ]
                ...
                for path in possible_paths:
                    ...
                    rospy.logerr("No se pudo encontrar el script detect_timed.py. Por favor especifica la ruta completa.")
                    return
            
            # Preparar comando YOLO
            yolo_cmd = [
                "python3", 
                yolo_script_path, 
            ...
            # Iniciar proceso YOLO
            rospy.loginfo(f"Iniciando detector YOLO: {' '.join(yolo_cmd)}")
            ...
            # Iniciar un thread para monitorear la salida de YOLO
            import threading
            def monitor_yolo_output():
                for line in iter(self.yolo_process.stdout.readline, ''):
            ...
            rospy.loginfo("Detector YOLO iniciado correctamente")
        except Exception as e:
            rospy.logerr(f"Error al iniciar el detector YOLO: {e}")
            self.yolo_process = None

    def stop_yolo_detector(self):
        """Detener el proceso del detector YOLO de forma segura"""
        if self.yolo_process:
            rospy.loginfo("Deteniendo el detector YOLO...")
            try:
                # Enviar SIGTERM primero para cierre limpio
                ...
                try:
                    ...
                    rospy.logwarn("YOLO no respondió a SIGTERM, enviando SIGKILL")
                    self.yolo_process.kill()
                    self.yolo_process.wait(timeout=2)
                
                rospy.loginfo("Detector YOLO detenido correctamente")
            except Exception as e:
                rospy.logerr(f"Error al detener el detector YOLO: {e}")
            finally:
                self.yolo_process = None
